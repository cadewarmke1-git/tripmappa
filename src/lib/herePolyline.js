/** HERE flexible polyline decoder for client-side route rendering. */
...
export function decodeFlexiblePolyline(encoded) {
  ...
  for (; i < decoder.length;) {
    ...
    if (thirdDim) {
      ...
      points.push({ lat: lastLat / factorDegree, lng: lastLng / factorDegree });
      i += 3;
    } else {
      points.push({ lat: lastLat / factorDegree, lng: lastLng / factorDegree });
      i += 2;
...
export function decodeHerePolylines(polylines = []) {
  const merged = [];
  for (const encoded of polylines) {
    const pts = decodeFlexiblePolyline(encoded);
    for (const pt of pts) {
