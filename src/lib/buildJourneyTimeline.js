...
/** Build timeline rows from itinerary waypoints + route leg summaries. */
export function buildJourneyTimelineFromWaypoints(waypoints = [], { routeLegs = [], departureTime = null } = {}) {
  const rows = [];
  const visible = waypoints.filter(w => {
    if (w.kind === "origin") return false;
    if (w.kind === "stop" && !w.included) return false;
    return true;
  });
  ...
  visible.forEach((w, index) => {
    ...
    if (w.kind === "destination") {
      ...
      rows.push({
        kind: "stop",
        id: w.id,
        stop: waypointToStop(w),
        isOvernight: w.role === "overnight",
        dayIndex: 0,
        sortable: true,
      });
    }
  ...
  if (!rows.some(r => r.kind === "destination") && waypoints.length) {
    const dest = waypoints.find(wp => wp.kind === "destination");
    if (dest) {
      rows.push({
        kind: "destination",
        id: dest.id,
