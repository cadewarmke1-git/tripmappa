  ...
  const fetchDirections = useCallback((vehicleType) => {
    ...
    if (cached) {
      ...
      setRouteInfo({
        ...
        arriveBy: timingMode === "arrive_by" ? arriveByDate : null,
        scenic,
        // Google cache — never claim truck-safe; only HERE sets truckSafe:true.
        truckSafe: false,
        rvSafe: false,
        truckHeight: answers.truck_height,
        truckWeight: answers.truck_weight,
        ...
        routeProvider: cached.routeInfo?.routeProvider || "google",
        hereRoute: null,
        herePolyline: null,
      });
      setOrigin(originVal);
      ...
      service.route(routeRequest, (result, status) => {
        ...
        if (status === "OK") {
          ...
          const nextRouteInfo = {
            ...
            arriveBy: timingMode === "arrive_by" ? arriveByDate : null,
            scenic,
            // Google Directions is not truck-validated — only HERE sets truckSafe:true.
            truckSafe: false,
            rvSafe: false,
            truckHeight: answers.truck_height,
            truckWeight: answers.truck_weight,
